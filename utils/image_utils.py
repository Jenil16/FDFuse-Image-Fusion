...
def YCbCr2RGB(Y,Cb,Cr):
    ...
    # Adjust Cb and Cr back from the 0.5 bias added during RGB2YCbCr
    Cb_adj = Cb - 0.5
    Cr_adj = Cr - 0.5

    # YCbCr to RGB inverse conversion formulas
    R = Y + 1.403 * Cr_adj
    G = Y - 0.714 * Cr_adj - 0.344 * Cb_adj
    B = Y + 1.773 * Cb_adj
    ...
    # Concatenate R, G, B channels back to form the RGB image tensor
    rgb_image = torch.cat([R, G, B], dim=1) # Concatenate along the channel dimension
...
def claheTensor(image, clip_limit=1.0, tile_grid_size=(8, 8)):
    image_np = image.cpu().detach().numpy()
    for i in range(image_np.shape[0]):
        img = (image_np[i, 0, :, :] * 255)
        clahe = cv2.createCLAHE(clipLimit=clip_limit, tileGridSize=tile_grid_size) #clipLimit=4.0, tileGridSize=(8, 8)
