...
class FeedForward(nn.Module):   #GDFN
...
class Attention(nn.Module):  #MDTA
...
class TransformerBlock(nn.Module):  #Encoder(RestomerBlock -> MDTA + GDFN)
...
class StarBlock(nn.Module):  #Figure 1 starblock architecture (specific features)
