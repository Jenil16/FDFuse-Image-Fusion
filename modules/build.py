...
class ShareFuse(nn.Module):
    ...
    def forward(self, vi, ir):
        ...
        out = (vi + ir) / 2   # Mean for shared features
        return out
...
class PrivateFuse(nn.Module):
    ...
    def forward(self, vi, ir):
        ...
        ir = ir + self.ffn_ir(self.norm2_ir(ir))
        out = torch.max(vi, ir)    # max for specific features
        return out
...
class Decoder(nn.Module):
    ...
    def forward(self,share_feats,private_feats):   
        #share_feats (shared features) - from restormer block(TransformerBlock X4), 
        #private_feats (specific features) - from Starblock X2
        out_feats = torch.cat([share_feats,private_feats],dim=1)  #concatenation of both shared & specific features
        out_enc_level0 = self.reduce_channel(out_feats)  #Conv layer
        out_enc_level1 = self.encoder_level2(out_enc_level0)  #restomer block X2
        out_enc_level1 = self.output(out_enc_level1) #Conv - ReLU - Conv 
        return self.sigmoid(out_enc_level1) #Sigmoid
